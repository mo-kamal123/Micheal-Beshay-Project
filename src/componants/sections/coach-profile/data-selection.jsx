    ...
    <div className="flex flex-col-reverse lg:flex-row gap-6 p-6 bg-white rounded-2xl w-[95%] md:w-[90%] my-20 m-auto min-h-screen">
          ...
          {dates.map((d, i) => (
            <button
              ...
              className={`px-6 py-4 text-center border border-[#DCE0E5] rounded-2xl  ${
                selectedDate.endsWith(d.date.toString())
                  ? "bg-main text-white"
                  : "bg-white"
              }`}
            >
      ...
      <div className="w-full h-fit bg-white border border-[#E2E2E2] rounded p-4 shadow">
        <div className="flex justify-between items-center gap-4 mb-4">
          ...
          <div>
            <p> session with <strong>(Amand Clar)</strong></p>
            <p className="text-sm text-gray-500">Session: 1</p>
          </div>
          <span className="text-main font-bold">300 <span className="text-[8px]"> EGP </span></span>
        </div>
        <Link to={'/confirm-booking'} className="w-full bg-main text-white flex justify-center py-2 rounded ">
          Book your time
        </Link>
      </div>
    </div>
